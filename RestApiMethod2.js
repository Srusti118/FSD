...
const router = express.Router() ; //express ma chhe but need to be called explicity , capital ma chhe
const students=[
...
{
  var cd = students.filter((s) => s.rollno == req.params.roll) 
    if (cd.length> 0)
        res.send(cd);
    else
        res.send(`<h3>No such data exists</h3>`)
});

router.get("/div/:div/test/:test" , (req,res) =>
{

  var cd = students.filter((s) => ((s.div == req.params.div) && (s.test == req.params.test))) 
    if (cd.length> 0)
        res.send(cd);
    else
        res.send(`<h3>No such data exists</h3>`)
